...
public class RequestValidationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(final ServerWebExchange exchange, final GatewayFilterChain chain) {
        if(rateLimiterProperty.getRateSwitch()){  // 开启限流
            try{
                rateLimiter.acquire();
    ...
    private Map<String, String> doExecute(String originalBody, ServerWebExchange exchange) {
        ...
        if(checkParameter(originalBody, noVerify) && !skipCheckParameter(url)) {  // 进行参数验证
            ...
            if(StringUtil.isNotEmpty(encrypt) && V2.equals(encrypt)){  // 版本2 参数加密并签名
                String decrypt = RsaTool.decrypt(bodyContent.get(BUSINESS_BODY), channelDataVo.getDataSecretKey()); // 使用私钥解密数据
                bodyContent.put(BUSINESS_BODY, decrypt);
            }
            boolean checkFlag = RsaSignTool.verifyRsaSign256(bodyContent, channelDataVo.getSignPublicKey()); // 验签
            ...
            if(!skipCheckTokenResult && StringUtil.isEmpty(token)){
                ...
                argumentError.setMsg("token参数为空");
